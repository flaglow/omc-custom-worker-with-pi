...
- **Shell interpolation safety**: Values passed to `node -e` use `process.argv` or env vars, never inline string interpolation. Pi worker spawn uses temp-file launcher scripts with env vars instead of `printf %q` shell escaping, so task text is never exposed to shell parsing. Temp files are cleaned up via trap on EXIT.
